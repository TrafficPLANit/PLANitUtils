 ...
 * Objects of some type. Data container used is a raw array of a fixed size based on the Od number of zones. Cells
 * that have no value entry will still take up space in this implementation. If the matrix is dense and contains
 * very little empty cells, this is likely the most computationally efficient implementation, if not, consider
 * other implementations that deal better with sparse matrices in terms of memory usage.
 ...
 *
 */
public abstract class ZoneToZoneNonPrimitiveMatrix<T> extends ZoneToZoneMatrixImpl<T, T[][]> {

  /** the logger */
  @SuppressWarnings("unused")
  private static final Logger LOGGER = Logger.getLogger(ZoneToZoneNonPrimitiveMatrix.class.getCanonicalName());
   ...
   */
  public ZoneToZoneNonPrimitiveMatrix(ZoneToZoneNonPrimitiveMatrix<T> other) {
    super(other);
  }
  ...
  public void setValue(Zone from, Zone to, T value) {
    matrixContainer[(int) from.getId()][(int) to.getId()] = value;
  }
  ...
  public T getValue(Zone from, Zone to) {
    return matrixContainer[(int) from.getId()][(int) to.getId()];
  }
  ...
  public T getValue(long from, long to) {
    return matrixContainer[(int) from][(int) to];
  }
   ...
   */
  @Override
  public abstract @NonNull ZoneToZoneMatrixIterator<T, T[][]> iterator();
}
